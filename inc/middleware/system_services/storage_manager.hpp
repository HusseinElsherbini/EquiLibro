// In a new file: storage_manager.hpp
#pragma once

#include "platform.hpp"
#include "hardware_abstraction/flash.hpp"
#include "common/platform_flash.hpp"
#include "sbr_app_types.hpp" // For CalibrationData structure
...
// Storage management status codes
enum class StorageStatus {
    OK,                 // Operation successful
    NOT_INITIALIZED,    // Storage system not initialized
    NO_DATA_FOUND,      // No valid data found
    INVALID_DATA,       // Data found but invalid
    WRITE_ERROR,        // Error writing to storage
    ERASE_ERROR,        // Error erasing storage
    READ_ERROR          // Error reading from storage
};
...
constexpr uint32_t CALIBRATION_STORAGE_SECTOR = 5;      // Use the last sector (128KB)
...
private:
    ...
    uint32_t CalculateCRC32(const void* data, uint32_t size);
    
    // Get absolute address for calibration storage
    uint32_t GetCalibrationStorageAddress();
    
    // Check if storage is initialized
    bool initialized;
    
    // Flash interface reference
    Platform::FLASH::FlashInterface& flash;
    
    // Cached sector information
    Platform::FLASH::FlashSector calibration_sector;
};
