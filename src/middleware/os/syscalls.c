/* Includes */
#include <errno.h>
#include <signal.h>
#include <stdio.h>
#include <stdlib.h>
#include <sys/stat.h>
#include <sys/time.h>
#include <sys/times.h>
#include <time.h>

/* Variables */
// #undef errno
extern int errno;
extern int
__io_putchar(int ch) __attribute__((weak));
extern int
__io_getchar(void) __attribute__((weak));

register char* stack_ptr asm("sp");

char* __env[1] = { 0 };
char** environ = __env;

/* Functions */
void
initialise_monitor_handles()
{
}

int
_getpid(void)
{
  return 1;
}

int
_kill(int pid, int sig)
{
  (void)pid; // Explicitly mark as unused
  (void)sig; // Explicitly mark as unused
  errno = EINVAL;
  return -1;
}

void
_exit(int status)
{
  _kill(status, -1);
  while (1) {
  } /* Make sure we hang here */
}

__attribute__((weak)) int
_read(int file, char* ptr, int len)
{
  (void)file; // Explicitly mark as unused
  int DataIdx;

  for (DataIdx = 0; DataIdx < len; DataIdx++) {
    *ptr++ = __io_getchar();
  }

  return len;
}

__attribute__((weak)) int
_write(int file, char* ptr, int len)
{
  (void)file; // Explicitly mark as unused
  int DataIdx;

  for (DataIdx = 0; DataIdx < len; DataIdx++) {
    __io_putchar(*ptr++);
    // ITM_SendChar(*ptr++);
  }
  return len;
}

int
_close(int file)
{
  (void)file; // Explicitly mark as unused
  return -1;
}

int
_fstat(int file, struct stat* st)
{
  (void)file; // Explicitly mark as unused
  st->st_mode = S_IFCHR;
  return 0;
}

int
_isatty(int file)
{
  (void)file; // Explicitly mark as unused
  return 1;
}

int
_lseek(int file, int ptr, int dir)
{
  (void)file; // Explicitly mark as unused
  (void)ptr;  // Explicitly mark as unused
  (void)dir;  // Explicitly mark as unused
  return 0;
}

int
_open(char* path, int flags, ...)
{
  (void)path;  // Explicitly mark as unused
  (void)flags; // Explicitly mark as unused
  /* Pretend like we always fail */
  return -1;
}

int
_wait(int* status)
{
  (void)status; // Explicitly mark as unused
  errno = ECHILD;
  return -1;
}

int
_unlink(char* name)
{
  (void)name; // Explicitly mark as unused
  errno = ENOENT;
  return -1;
}

int
_times(struct tms* buf)
{
  (void)buf; // Explicitly mark as unused
  return -1;
}

int
_stat(char* file, struct stat* st)
{
  (void)file; // Explicitly mark as unused
  st->st_mode = S_IFCHR;
  return 0;
}

int
_link(char* old, char* new)
{
  (void)old; // Explicitly mark as unused
  (void)new; // Explicitly mark as unused
  errno = EMLINK;
  return -1;
}

int
_fork(void)
{

  errno = EAGAIN;
  return -1;
}

int
_execve(char* name, char** argv, char** env)
{
  (void)name; // Explicitly mark as unused
  (void)argv; // Explicitly mark as unused
  (void)env;  // Explicitly mark as unused
  errno = ENOMEM;
  return -1;
}

/**
 _sbrk
 Increase program data space. Malloc and related functions depend on this
**/
caddr_t
_sbrk(int incr)
{
  extern char end asm("end");
  static char* heap_end;
  char* prev_heap_end;

  if (heap_end == 0)
    heap_end = &end;

  prev_heap_end = heap_end;
  if (heap_end + incr > stack_ptr) {
    errno = ENOMEM;
    return (caddr_t)-1;
  }

  heap_end += incr;

  return (caddr_t)prev_heap_end;
}
